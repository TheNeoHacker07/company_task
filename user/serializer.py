...
from .email import send_activation_code  #импорт функции из файла email.py для отправки активационного кода по имейлу 
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    # метод для проверки и валидации пароля
    def validate(self, attrs):
        password = attrs.get('password')
        password_confirm = attrs.pop('password_confirm')

        #Проверка пароля на то что совпадают ли они
        if password != password_confirm:
            raise serializers.ValidationError('Пароли не совпадают!')
        
        #Проверка на надежность пароля на всякий случий
